local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_tale_of_tin_viel"
CARD.Description = "ce_expansion_magic_ltr_tale_of_tin_viel_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/tale_of_tin_viel"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON_ROUNDED
CARD.Attributes = {
        ManaCost = "{3}{W}{W}",
        Cmc = 5,
        Colors = {
            "W"
        },
        ColorIdentity = {
            "W"
        },
        Type = "Enchantment — Saga",
        Types = {
            "Enchantment"
        },
        Subtypes = {
            "Saga"
        },
        Rarity = "Uncommon",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — Target creature you control gains indestructible for as long as you control Tale of Tinúviel.\nII — Return target creature card from your graveyard to the battlefield.\nIII — Up to two target creatures you control each gain lifelink until end of turn.",
        Artist = "Anthony Devine",
        Number = "34",
        Layout = "saga",
        Multiverseid = "616864",
        Variations = {
            "9825f815-d58a-565e-84c8-242a87ce9058"
        },
        ForeignNames = {
            {
                Name = "Geschichte von Tinúviel",
                Text = "(Je eine Sagenmarke beim Ins-Spiel-Kommen und nach deinem Ziehsegment. Opfern nach III.)\nI — Eine Kreatur deiner Wahl, die du kontrollierst, erhält Unzerstörbarkeit, solange du die Geschichte von Tinúviel kontrollierst.\nII — Bringe eine Kreaturenkarte deiner Wahl aus deinem Friedhof ins Spiel zurück.\nIII — Bis zu zwei Kreaturen deiner Wahl, die du kontrollierst, erhalten Lebensverknüpfung bis zum Ende des Zuges.",
                Type = "Verzauberung — Sage",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617125&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "15e373b8-c53f-4b87-bc46-6f6334686da4",
                    MultiverseId = 617125
                },
                Multiverseid = 617125
            },
            {
                Name = "La historia de Tinúviel",
                Text = "(En cuanto esta Saga entre y después de tu paso de robar, agrega un contador de sabiduría. Sacrifícala después de III.)\nI — La criatura objetivo que controlas gana la habilidad de indestructible mientras controles La historia de Tinúviel.\nII — Regresa la carta de criatura objetivo de tu cementerio al campo de batalla.\nIII — Hasta dos criaturas objetivo que controlas ganan la habilidad de vínculo vital hasta el final del turno.",
                Type = "Encantamiento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617386&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "7bad3693-211e-4efb-94a2-5c271d5d6d06",
                    MultiverseId = 617386
                },
                Multiverseid = 617386
            },
            {
                Name = "Conte de Tinúviel",
                Text = "(Au moment où cette saga arrive sur le champ de bataille et après votre étape de pioche, ajoutez un marqueur « sapience ». Sacrifiez après III.)\nI — Une créature ciblée que vous contrôlez acquiert l'indestructible tant que vous contrôlez le Conte de Tinúviel.\nII — Renvoyez sur le champ de bataille une carte de créature ciblée depuis votre cimetière.\nIII — Jusqu'à deux créatures ciblées que vous contrôlez acquièrent chacune le lien de vie jusqu'à la fin du tour.",
                Type = "Enchantement : saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617647&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "8b59a625-20b4-4356-bfe5-a24bb0f3d885",
                    MultiverseId = 617647
                },
                Multiverseid = 617647
            },
            {
                Name = "Storia di Tinúviel",
                Text = "(Mentre questa Saga entra e dopo la tua sottofase di acquisizione, aggiungi un segnalino sapere. Sacrifica dopo III.)\nI — Una creatura bersaglio che controlli ha indistruttibile fintanto che controlli la Storia di Tinúviel.\nII — Rimetti sul campo di battaglia una carta creatura bersaglio dal tuo cimitero.\nIII — Fino a due creature bersaglio che controlli hanno legame vitale fino alla fine del turno.",
                Type = "Incantesimo — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617908&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "2f784340-4b9f-464b-9cf8-b2da5e425ee9",
                    MultiverseId = 617908
                },
                Multiverseid = 617908
            },
            {
                Name = "ティヌーヴィエルの物語",
                Text = "（この英雄譚が出た際とあなたのドロー・ステップの後に、伝承カウンター１個を加える。IIIの後に、生け贄に捧げる。）\nI — あなたがコントロールしているクリーチャー１体を対象とする。あなたがティヌーヴィエルの物語をコントロールし続けているかぎり、それは破壊不能を得る。\nII — あなたの墓地にあるクリーチャー・カード１枚を対象とする。それを戦場に戻す。\nIII — あなたがコントロールしているクリーチャー最大２体を対象とする。ターン終了時まで、それらは絆魂を得る。",
                Type = "エンチャント — 英雄譚",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618169&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "df0b1946-42e6-4e17-824c-ed38fb41faad",
                    MultiverseId = 618169
                },
                Multiverseid = 618169
            },
            {
                Name = "História de Tinúviel",
                Text = "(Conforme esta Saga entra e após sua etapa de compra, adicione um marcador de conhecimento. Sacrifique-a após III.)\nI — A criatura alvo que você controla ganha indestrutível enquanto você controlar História de Tinúviel.\nII — Devolva o card de criatura alvo de seu cemitério para o campo de batalha.\nIII — Até duas criaturas alvo que você controla ganham cada uma vínculo com a vida até o final do turno.",
                Type = "Encantamento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618430&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "693b1306-e33f-44e1-9555-28dc889f52b1",
                    MultiverseId = 618430
                },
                Multiverseid = 618430
            },
            {
                Name = "缇努维尔的故事",
                Text = "（于此传纪进战场时及于你抓牌步骤后，加一个学问指示物。到III后牺牲之。）\nI — 于你操控缇努维尔的故事之时段内，目标由你操控的生物获得不灭异能。\nII — 将目标生物牌从你的坟墓场移回战场。\nIII — 至多两个目标由你操控的生物各获得系命异能直到回合结束。",
                Type = "结界 ～传纪",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618691&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "74340c1b-0d73-40a1-a2da-26d46165f513",
                    MultiverseId = 618691
                },
                Multiverseid = 618691
            }
        },
        Printings = {
            "LTR"
        },
        OriginalText = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — Target creature you control gains indestructible for as long as you control Tale of Tinúviel.\nII — Return target creature card from your graveyard to the battlefield.\nIII — Up to two target creatures you control each gain lifelink until end of turn.",
        OriginalType = "Enchantment — Saga",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "3a018e50-684e-531f-b1c3-08f674a713a9"
    }
