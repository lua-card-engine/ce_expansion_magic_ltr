local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_fall_of_gil_galad"
CARD.Description = "ce_expansion_magic_ltr_fall_of_gil_galad_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/fall_of_gil_galad"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON_ROUNDED
CARD.Attributes = {
        ManaCost = "{1}{G}",
        Cmc = 2,
        Colors = {
            "G"
        },
        ColorIdentity = {
            "G"
        },
        Type = "Enchantment — Saga",
        Types = {
            "Enchantment"
        },
        Subtypes = {
            "Saga"
        },
        Rarity = "Rare",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — Scry 2.\nII — Put two +1/+1 counters on target creature you control.\nIII — Until end of turn, target creature you control gains \"When this creature dies, draw two cards.\" Then that creature fights up to one other target creature.",
        Artist = "Craig Elliott",
        Number = "165",
        Layout = "saga",
        Multiverseid = "616995",
        Variations = {
            "d45aebf8-bd0f-5804-9869-4a427fd52544"
        },
        ForeignNames = {
            {
                Name = "Gil-galads Untergang",
                Text = "(Je eine Sagenmarke beim Ins-Spiel-Kommen und nach deinem Ziehsegment. Opfern nach III.)\nI — Hellsicht 2.\nII — Lege zwei +1/+1-Marken auf eine Kreatur deiner Wahl, die du kontrollierst.\nIII — Bis zum Ende des Zuges erhält eine Kreatur deiner Wahl, die du kontrollierst, „Wenn diese Kreatur stirbt, ziehe zwei Karten.\" Dann kämpft jene Kreatur gegen bis zu eine andere Kreatur deiner Wahl.",
                Type = "Verzauberung — Sage",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617256&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "175f9fc4-a884-45cc-835e-8cb13f27910f",
                    MultiverseId = 617256
                },
                Multiverseid = 617256
            },
            {
                Name = "La caída de Gil-galad",
                Text = "(En cuanto esta Saga entre y después de tu paso de robar, agrega un contador de sabiduría. Sacrifícala después de III.)\nI — Adivina 2.\nII — Pon dos contadores +1/+1 sobre la criatura objetivo que controlas.\nIII — Hasta el final del turno, la criatura objetivo que controlas gana \"Cuando esta criatura muera, roba dos cartas\". Luego, esa criatura lucha contra hasta una otra criatura objetivo.",
                Type = "Encantamiento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617517&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "e880a458-94f5-470f-ae81-0daf8c502470",
                    MultiverseId = 617517
                },
                Multiverseid = 617517
            },
            {
                Name = "Chute de Gil-galad",
                Text = "(Au moment où cette saga arrive sur le champ de bataille et après votre étape de pioche, ajoutez un marqueur « sapience ». Sacrifiez après III.)\nI — Regard 2.\nII — Mettez deux marqueurs +1/+1 sur une créature ciblée que vous contrôlez.\nIII — Jusqu'à la fin du tour, une créature ciblée que vous contrôlez acquiert « Quand cette créature meurt, piochez deux cartes. » Puis cette créature se bat contre jusqu'à une autre créature ciblée.",
                Type = "Enchantement : saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617778&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "57f84318-bfeb-47ee-b05e-2deed4db4b7b",
                    MultiverseId = 617778
                },
                Multiverseid = 617778
            },
            {
                Name = "Caduta di Gil-galad",
                Text = "(Mentre questa Saga entra e dopo la tua sottofase di acquisizione, aggiungi un segnalino sapere. Sacrifica dopo III.)\nI — Profetizza 2.\nII — Metti due segnalini +1/+1 su una creatura bersaglio che controlli.\nIII —Fino alla fine del turno, una creatura bersaglio che controlli ha \"Quando questa creatura muore, pesca due carte\". Poi quella creatura lotta con fino a un'altra creatura bersaglio.",
                Type = "Incantesimo — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618039&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "6537984e-953e-4318-93b9-33ba6be0725b",
                    MultiverseId = 618039
                },
                Multiverseid = 618039
            },
            {
                Name = "ギル＝ガラドの討死",
                Text = "（この英雄譚が出た際とあなたのドロー・ステップの後に、伝承カウンター１個を加える。IIIの後に、生け贄に捧げる。）\nI ― 占術２を行う。\nII ― あなたがコントロールしているクリーチャー１体を対象とする。それの上に＋１/＋１カウンター２個を置く。\nIII ― あなたがコントロールしているクリーチャー１体と、それでないクリーチャー最大１体を対象とする。ターン終了時まで、その前者は「このクリーチャーが死亡したとき、カード２枚を引く。」を得る。その後、その前者はその後者と格闘を行う。",
                Type = "エンチャント — 英雄譚",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618300&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "a0c418f5-2e6e-4cdf-b508-0d1cb90a44b6",
                    MultiverseId = 618300
                },
                Multiverseid = 618300
            },
            {
                Name = "Queda de Gil-galad",
                Text = "(Conforme esta Saga entra e após sua etapa de compra, adicione um marcador de conhecimento. Sacrifique-a após III.)\nI — Use vidência 2.\nII — Coloque dois marcadores +1/+1 na criatura alvo que você controla.\nIII — Até o final do turno, a criatura alvo que você controla ganha \"Quando esta criatura morrer, compre dois cards\". Depois, aquela criatura luta contra até uma outra criatura alvo.",
                Type = "Encantamento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618561&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "bc3e34f2-20f4-4bbb-a139-e47f1164587d",
                    MultiverseId = 618561
                },
                Multiverseid = 618561
            },
            {
                Name = "吉尔加拉德陨落",
                Text = "（于此传纪进战场时及于你抓牌步骤后，加一个学问指示物。到III后牺牲之。）\nI — 占卜2。\nII — 在目标由你操控的生物上放置两个+1/+1指示物。\nIII — 直到回合结束，目标由你操控的生物获得「当此生物死去时，抓两张牌。」然后该生物与至多另一个目标生物互斗。",
                Type = "结界 ～传纪",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618822&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "ee6775f0-379d-43d4-b19b-492a7ac7ec99",
                    MultiverseId = 618822
                },
                Multiverseid = 618822
            }
        },
        Printings = {
            "LTR",
            "PLTR"
        },
        OriginalText = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — Scry 2.\nII — Put two +1/+1 counters on target creature you control.\nIII — Until end of turn, target creature you control gains \"When this creature dies, draw two cards.\" Then that creature fights up to one other target creature.",
        OriginalType = "Enchantment — Saga",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Penny",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "2f423dda-aa05-5bab-9b85-9c83a19f1a19"
    }
