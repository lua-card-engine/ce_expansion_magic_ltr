local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_one_ring_to_rule_them_all"
CARD.Description = "ce_expansion_magic_ltr_one_ring_to_rule_them_all_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/one_ring_to_rule_them_all"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON
CARD.Attributes = {
        ManaCost = "{2}{B}{B}",
        Cmc = 4,
        Colors = {
            "B"
        },
        ColorIdentity = {
            "B"
        },
        Type = "Enchantment — Saga",
        Types = {
            "Enchantment"
        },
        Subtypes = {
            "Saga"
        },
        Rarity = "Rare",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — The Ring tempts you, then each player mills cards equal to your Ring-bearer's power.\nII — Destroy all nonlegendary creatures.\nIII — Each opponent loses 1 life for each creature card in that player's graveyard.",
        Artist = "L J Koh",
        Number = "102",
        Layout = "saga",
        Multiverseid = "616932",
        Variations = {
            "15a8f0eb-5450-5689-8adb-14b524606f13"
        },
        Rulings = {
            {
                Date = "2023-06-16",
                Text = "As the Ring tempts you, you get an emblem named The Ring if you don't have one. Then your emblem gains its next ability and you choose a creature you control to become (or remain) your Ring-bearer."
            },
            {
                Date = "2023-06-16",
                Text = "Each player can have only one emblem named The Ring and only one Ring-bearer at a time."
            },
            {
                Date = "2023-06-16",
                Text = "Each time the Ring tempts you, you must choose a creature if you control one."
            },
            {
                Date = "2023-06-16",
                Text = "If the creature you choose as your Ring-bearer was already your Ring-bearer, that still counts as choosing that creature as your Ring-bearer for the purpose of abilities that trigger \"whenever you choose a creature as your Ring-bearer\" or abilities that care about which creature was chosen as your Ring-bearer."
            },
            {
                Date = "2023-06-16",
                Text = "Some spells and abilities that cause the Ring to tempt you may require targets. If each target chosen is an illegal target as that spell or ability tries to resolve, it won't resolve. The Ring won't tempt you."
            },
            {
                Date = "2023-06-16",
                Text = "The Ring can tempt you even if you don't control a creature. In this case, abilities that trigger \"whenever the Ring tempts you\" will still trigger."
            },
            {
                Date = "2023-06-16",
                Text = "The Ring gains its abilities in order from top to bottom. Once it gains an ability, it has that ability for the rest of the game."
            }
        },
        ForeignNames = {
            {
                Name = "Ein Ring, sie alle zu knechten",
                Text = "(Je eine Sagenmarke beim Ins-Spiel-Kommen und nach deinem Ziehsegment. Opfern nach III.)\nI — Der Ring führt dich in Versuchung, dann millt jeder Spieler so viele Karten, wie die Stärke deines Ringträgers beträgt.\nII — Zerstöre alle nichtlegendären Kreaturen.\nIII — Jeder Gegner verliert für jede Kreaturenkarte in seinem Friedhof 1 Lebenspunkt.",
                Type = "Verzauberung — Sage",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617193&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "36c054f2-2aaa-4ac7-8737-c06721d7f7e2",
                    MultiverseId = 617193
                },
                Multiverseid = 617193
            },
            {
                Name = "Un Anillo para gobernarlos a todos",
                Text = "(En cuanto esta Saga entre y después de tu paso de robar, agrega un contador de sabiduría. Sacrifícala después de III.)\nI — El Anillo te tienta, luego cada jugador muele una cantidad de cartas igual a la fuerza de tu portador del Anillo.\nII — Destruye todas las criaturas que no sean legendarias.\nIII — Cada oponente pierde 1 vida por cada carta de criatura en el cementerio de ese jugador.",
                Type = "Encantamiento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617454&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "30f51566-e41c-4ee6-9891-f2ac2f9076f4",
                    MultiverseId = 617454
                },
                Multiverseid = 617454
            },
            {
                Name = "Un Anneau pour les gouverner tous",
                Text = "(Au moment où cette saga arrive sur le champ de bataille et après votre étape de pioche, ajoutez un marqueur « sapience ». Sacrifiez après III.)\nI — L'Anneau vous tente, puis chaque joueur meule un nombre de cartes égal à la force de votre porteur de l'Anneau.\nII — Détruisez toutes les créatures non-légendaires.\nIII — Chaque adversaire perd 1 point de vie pour chaque carte de créature dans le cimetière de ce joueur.",
                Type = "Enchantement : saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617715&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "fd96a5e7-7e9a-438d-aefa-9daf58b22b67",
                    MultiverseId = 617715
                },
                Multiverseid = 617715
            },
            {
                Name = "Un Anello per Domarli",
                Text = "(Mentre questa Saga entra e dopo la tua sottofase di acquisizione, aggiungi un segnalino sapere. Sacrifica dopo III.)\nI — L'Anello ti tenta, poi ogni giocatore macina carte pari alla forza del tuo Portatore dell'Anello.\nII — Distruggi tutte le creature non leggendarie.\nII — Ogni avversario perde 1 punto vita per ogni carta creatura nel proprio cimitero.",
                Type = "Incantesimo — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617976&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "4831ac1b-2888-433f-8d3c-25c0c2a15fd3",
                    MultiverseId = 617976
                },
                Multiverseid = 617976
            },
            {
                Name = "一つの指輪は、すべてを統べ",
                Text = "（この英雄譚が出た際とあなたのドロー・ステップの後に、伝承カウンター１個を加える。IIIの後に、生け贄に捧げる。）\nI — 指輪があなたを誘惑する。その後、各プレイヤーはそれぞれ、あなたの指輪所持者のパワーに等しい枚数のカードを切削する。\nII — 伝説でないすべてのクリーチャーを破壊する。\nIII — 各対戦相手はそれぞれ、自分の墓地にあるクリーチャー・カード１枚につき１点のライフを失う。",
                Type = "エンチャント — 英雄譚",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618237&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "25f0a537-f9a2-496b-a3a9-bfdcd1d720c2",
                    MultiverseId = 618237
                },
                Multiverseid = 618237
            },
            {
                Name = "Um Anel Para Todos Governar",
                Text = "(Conforme esta Saga entra e após sua etapa de compra, adicione um marcador de conhecimento. Sacrifique-a após III.)\nI — O Anel tenta você e depois cada jogador tritura uma quantidade de cards igual ao poder de seu Guardião do Anel.\nII — Destrua todas as criaturas não lendárias.\nIII — Cada oponente perde 1 ponto de vida para cada card de criatura no cemitério daquele jogador.",
                Type = "Encantamento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618498&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "66fb2fa3-d10e-4485-906d-eeddddcff0fd",
                    MultiverseId = 618498
                },
                Multiverseid = 618498
            },
            {
                Name = "至尊魔戒御余众",
                Text = "（于此传纪进战场时及于你抓牌步骤后，加一个学问指示物。到III后牺牲之。）\nI — 魔戒引诱你，然后每位牌手各磨若干牌，其数量等同于你持戒人的力量。\nII — 消灭所有非传奇生物。\nIII — 对每位对手而言，该牌手坟墓场中每有一张生物牌，其便失去1点生命。",
                Type = "结界 ～传纪",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618759&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "cf7ff8d7-7df8-441a-a394-303b3426fed9",
                    MultiverseId = 618759
                },
                Multiverseid = 618759
            }
        },
        Printings = {
            "LTR",
            "PLTR"
        },
        OriginalText = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — The Ring tempts you, then each player mills cards equal to your Ring-bearer's power.\nII — Destroy all nonlegendary creatures.\nIII — Each opponent loses 1 life for each creature card in that player's graveyard.",
        OriginalType = "Enchantment — Saga",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Penny",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "3428febd-4e31-56ff-8dfc-8e3dd035d07b"
    }
