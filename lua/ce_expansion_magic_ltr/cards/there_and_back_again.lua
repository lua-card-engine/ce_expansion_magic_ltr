local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_there_and_back_again"
CARD.Description = "ce_expansion_magic_ltr_there_and_back_again_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/there_and_back_again"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON
CARD.Attributes = {
        ManaCost = "{3}{R}{R}",
        Cmc = 5,
        Colors = {
            "R"
        },
        ColorIdentity = {
            "R"
        },
        Type = "Enchantment — Saga",
        Types = {
            "Enchantment"
        },
        Subtypes = {
            "Saga"
        },
        Rarity = "Rare",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — Up to one target creature can't block for as long as you control There and Back Again. The Ring tempts you.\nII — Search your library for a Mountain card, put it onto the battlefield, then shuffle.\nIII — Create Smaug, a legendary 6/6 red Dragon creature token with flying, haste, and \"When this creature dies, create fourteen Treasure tokens.\"",
        Artist = "Jarel Threat",
        Number = "151",
        Layout = "saga",
        Multiverseid = "616981",
        Variations = {
            "f9918f42-8f0e-554e-b92d-1dfb3f93e38a"
        },
        Rulings = {
            {
                Date = "2023-06-16",
                Text = "As the Ring tempts you, you get an emblem named The Ring if you don't have one. Then your emblem gains its next ability and you choose a creature you control to become (or remain) your Ring-bearer."
            },
            {
                Date = "2023-06-16",
                Text = "Each player can have only one emblem named The Ring and only one Ring-bearer at a time."
            },
            {
                Date = "2023-06-16",
                Text = "Each time the Ring tempts you, you must choose a creature if you control one."
            },
            {
                Date = "2023-06-16",
                Text = "If the creature you choose as your Ring-bearer was already your Ring-bearer, that still counts as choosing that creature as your Ring-bearer for the purpose of abilities that trigger \"whenever you choose a creature as your Ring-bearer\" or abilities that care about which creature was chosen as your Ring-bearer."
            },
            {
                Date = "2023-06-16",
                Text = "Some spells and abilities that cause the Ring to tempt you may require targets. If each target chosen is an illegal target as that spell or ability tries to resolve, it won't resolve. The Ring won't tempt you."
            },
            {
                Date = "2023-06-16",
                Text = "The Ring can tempt you even if you don't control a creature. In this case, abilities that trigger \"whenever the Ring tempts you\" will still trigger."
            },
            {
                Date = "2023-06-16",
                Text = "The Ring gains its abilities in order from top to bottom. Once it gains an ability, it has that ability for the rest of the game."
            }
        },
        ForeignNames = {
            {
                Name = "Hin und zurück",
                Text = "(Je eine Sagenmarke beim Ins-Spiel-Kommen und nach deinem Ziehsegment. Opfern nach III.)\nI — Bis zu eine Kreatur deiner Wahl kann nicht blocken, solange du Hin und zurück kontrollierst. Der Ring führt dich in Versuchung.\nII — Durchsuche deine Bibliothek nach einer Gebirge-Karte, bringe sie ins Spiel und mische danach.\nIII — Erzeuge Smaug, einen legendären 6/6 roten Drache-Kreaturenspielstein mit Flugfähigkeit, Eile und „Wenn diese Kreatur stirbt, erzeuge vierzehn Schatz-Spielsteine.\"",
                Type = "Verzauberung — Sage",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617242&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "547bc5bb-ed13-4ca2-a2a8-7ffa4e15ef11",
                    MultiverseId = 617242
                },
                Multiverseid = 617242
            },
            {
                Name = "Historia de una ida y de una vuelta",
                Text = "(En cuanto esta Saga entre y después de tu paso de robar, agrega un contador de sabiduría. Sacrifícala después de III.)\nI — Hasta una criatura objetivo no puede bloquear mientras controles la Historia de una ida y de una vuelta. El Anillo te tienta.\nII — Busca en tu biblioteca una carta de Montaña, ponla en el campo de batalla y luego baraja.\nIII — Crea a Smaug, una ficha de criatura legendaria Dragón roja 6/6 con las habilidades de volar, prisa y \"Cuando esta criatura muera, crea catorce fichas de Tesoro\".",
                Type = "Encantamiento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617503&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "ce7346d7-7497-461b-9473-1959097b0840",
                    MultiverseId = 617503
                },
                Multiverseid = 617503
            },
            {
                Name = "Aller et retour",
                Text = "(Au moment où cette saga arrive sur le champ de bataille et après votre étape de pioche, ajoutez un marqueur « sapience ». Sacrifiez après III.)\nI — Jusqu'à une créature ciblée ne peut pas bloquer tant que vous contrôlez l'Aller et retour. L'Anneau vous tente.\nII — Cherchez dans votre bibliothèque une carte de montagne, mettez-la sur le champ de bataille, puis mélangez.\nIII — Créez Smaug, un jeton de créature légendaire 6/6 rouge Dragon avec le vol, la célérité et « Quand cette créature meurt, créez quatorze jetons Trésor. »",
                Type = "Enchantement : saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617764&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "ec5567f5-530f-4c57-a49c-edfa6dd70108",
                    MultiverseId = 617764
                },
                Multiverseid = 617764
            },
            {
                Name = "Andata e Ritorno",
                Text = "(Mentre questa Saga entra e dopo la tua sottofase di acquisizione, aggiungi un segnalino sapere. Sacrifica dopo III.)\nI — Fino a una creatura bersaglio non può bloccare fintanto che controlli Andata e Ritorno. L'Anello ti tenta.\nII — Passa in rassegna il tuo grimorio per una carta Montagna, mettila sul campo di battaglia, poi rimescola.\nIII — Crea Smaug, una pedina creatura leggendaria Drago 6/6 rossa con volare, rapidità e \"Quando questa creatura muore, crea quattordici pedine Tesoro\".",
                Type = "Incantesimo — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618025&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "f1bcb48a-42f0-45f9-87de-21dd81a88ad9",
                    MultiverseId = 618025
                },
                Multiverseid = 618025
            },
            {
                Name = "行きて帰りし物語",
                Text = "（この英雄譚が出た際とあなたのドロー・ステップの後に、伝承カウンター１個を加える。IIIの後に、生け贄に捧げる。）\nI — クリーチャー最大１体を対象とする。あなたが行きて帰りし物語をコントロールし続けているかぎり、それではブロックできない。指輪があなたを誘惑する。\nII — あなたのライブラリーから山・カード１枚を探し、戦場に出す。その後、ライブラリーを切り直す。\nIII — 「スマウグ」という名前で、飛行と速攻と「このクリーチャーが死亡したとき、宝物・トークン14個を生成する。」を持つ赤の６/６の伝説のドラゴン・クリーチャー・トークン１体を生成する。",
                Type = "エンチャント — 英雄譚",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618286&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "90ed35ac-f209-478c-8d1e-1729648d4a86",
                    MultiverseId = 618286
                },
                Multiverseid = 618286
            },
            {
                Name = "Lá e De Volta Outra Vez",
                Text = "(Conforme esta Saga entra e após sua etapa de compra, adicione um marcador de conhecimento. Sacrifique-a após III.)\nI — Até uma criatura alvo não pode bloquear enquanto você controlar Lá e de Volta Outra Vez. O Anel tenta você.\nII — Procure em seu grimório um card de Montanha, coloque-o no campo de batalha e depois embaralhe.\nIII — Crie Smaug, uma ficha de criatura lendária Dragão vermelha 6/6 com voar, ímpeto e \"Quando esta criatura morrer, crie quatorze fichas de Tesouro\".",
                Type = "Encantamento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618547&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "0d586426-5748-4def-a54a-547aabcd016d",
                    MultiverseId = 618547
                },
                Multiverseid = 618547
            },
            {
                Name = "去而复返",
                Text = "（于此传纪进战场时及于你抓牌步骤后，加一个学问指示物。到III后牺牲之。）\nI — 于你操控去而复返的时段内，至多一个目标生物不能进行阻挡。魔戒引诱你。\nII — 从你牌库中搜寻一张山脉牌，将之放进战场，然后洗牌。\nIII — 派出传奇衍生生物斯毛格，其为6/6红色的龙，且具有飞行、敏捷与「当此生物死去时，派出十四个珍宝衍生物。」",
                Type = "结界 ～传纪",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618808&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "5cb4137d-6528-43b3-b7af-160d88d85a3a",
                    MultiverseId = 618808
                },
                Multiverseid = 618808
            }
        },
        Printings = {
            "LTR",
            "PLTR"
        },
        OriginalText = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI — Up to one target creature can't block for as long as you control There and Back Again. The Ring tempts you.\nII — Search your library for a Mountain card, put it onto the battlefield, then shuffle.\nIII — Create Smaug, a legendary 6/6 red Dragon creature token with flying, haste, and \"When this creature dies, create fourteen Treasure tokens.\"",
        OriginalType = "Enchantment — Saga",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Penny",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "83d3f0fd-9148-57a7-9c93-e285418bf25f"
    }
