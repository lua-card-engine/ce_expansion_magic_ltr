local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_war_of_the_last_alliance"
CARD.Description = "ce_expansion_magic_ltr_war_of_the_last_alliance_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/war_of_the_last_alliance"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON_ROUNDED
CARD.Attributes = {
        ManaCost = "{3}{W}",
        Cmc = 4,
        Colors = {
            "W"
        },
        ColorIdentity = {
            "W"
        },
        Type = "Enchantment — Saga",
        Types = {
            "Enchantment"
        },
        Subtypes = {
            "Saga"
        },
        Rarity = "Rare",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI, II — Search your library for a legendary creature card, reveal it, put it into your hand, then shuffle.\nIII — Creatures you control gain double strike until end of turn. The Ring tempts you.",
        Artist = "Alexander Forssberg",
        Number = "36",
        Layout = "saga",
        Multiverseid = "616866",
        Variations = {
            "a815cd71-0b65-50d9-a2b2-40dd06231eba"
        },
        Rulings = {
            {
                Date = "2023-06-16",
                Text = "As the Ring tempts you, you get an emblem named The Ring if you don't have one. Then your emblem gains its next ability and you choose a creature you control to become (or remain) your Ring-bearer."
            },
            {
                Date = "2023-06-16",
                Text = "Each player can have only one emblem named The Ring and only one Ring-bearer at a time."
            },
            {
                Date = "2023-06-16",
                Text = "Each time the Ring tempts you, you must choose a creature if you control one."
            },
            {
                Date = "2023-06-16",
                Text = "If the creature you choose as your Ring-bearer was already your Ring-bearer, that still counts as choosing that creature as your Ring-bearer for the purpose of abilities that trigger \"whenever you choose a creature as your Ring-bearer\" or abilities that care about which creature was chosen as your Ring-bearer."
            },
            {
                Date = "2023-06-16",
                Text = "Some spells and abilities that cause the Ring to tempt you may require targets. If each target chosen is an illegal target as that spell or ability tries to resolve, it won't resolve. The Ring won't tempt you."
            },
            {
                Date = "2023-06-16",
                Text = "The Ring can tempt you even if you don't control a creature. In this case, abilities that trigger \"whenever the Ring tempts you\" will still trigger."
            },
            {
                Date = "2023-06-16",
                Text = "The Ring gains its abilities in order from top to bottom. Once it gains an ability, it has that ability for the rest of the game."
            }
        },
        ForeignNames = {
            {
                Name = "Krieg des Letzten Bündnisses",
                Text = "(Je eine Sagenmarke beim Ins-Spiel-Kommen und nach deinem Ziehsegment. Opfern nach III.)\nI, II — Durchsuche deine Bibliothek nach einer legendären Kreaturenkarte, zeige sie offen vor, nimm sie auf deine Hand und mische danach.\nIII — Kreaturen, die du kontrollierst, erhalten Doppelschlag bis zum Ende des Zuges. Der Ring führt dich in Versuchung.",
                Type = "Verzauberung — Sage",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617127&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "790fd516-e27a-41bd-881d-c7d0e1ac164d",
                    MultiverseId = 617127
                },
                Multiverseid = 617127
            },
            {
                Name = "Guerra de la Última Alianza",
                Text = "(En cuanto esta Saga entre y después de tu paso de robar, agrega un contador de sabiduría. Sacrifícala después de III.)\nI, II — Busca en tu biblioteca una carta de criatura legendaria, muéstrala, ponla en tu mano y luego baraja.\nIII — Las criaturas que controlas ganan la habilidad de dañar dos veces hasta el final del turno. El Anillo te tienta.",
                Type = "Encantamiento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617388&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "dedf47e7-cf1e-490b-9ee6-8670e9a23abf",
                    MultiverseId = 617388
                },
                Multiverseid = 617388
            },
            {
                Name = "Guerre de la Dernière Alliance",
                Text = "(Au moment où cette saga arrive sur le champ de bataille et après votre étape de pioche, ajoutez un marqueur « sapience ». Sacrifiez après III.)\nI, II — Cherchez dans votre bibliothèque une carte de créature légendaire, révélez-la, mettez-la dans votre main, puis mélangez.\nIII — Les créatures que vous contrôlez acquièrent la double initiative jusqu'à la fin du tour. L'Anneau vous tente.",
                Type = "Enchantement : saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617649&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "79843991-2af2-44fa-8389-122ec9e3cc2b",
                    MultiverseId = 617649
                },
                Multiverseid = 617649
            },
            {
                Name = "Guerra dell'Ultima Alleanza",
                Text = "(Mentre questa Saga entra e dopo la tua sottofase di acquisizione, aggiungi un segnalino sapere. Sacrifica dopo III.)\nI, II — Passa in rassegna il tuo grimorio per una carta creatura leggendaria, rivelala e aggiungila alla tua mano, poi rimescola.\nIII — Le creature che controlli hanno doppio attacco fino alla fine del turno. L'Anello ti tenta.",
                Type = "Incantesimo — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617910&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "0ddf87a5-6d5d-4b96-aab7-5cbf781c30c2",
                    MultiverseId = 617910
                },
                Multiverseid = 617910
            },
            {
                Name = "最後の同盟の戦い",
                Text = "（この英雄譚が出た際とあなたのドロー・ステップの後に、伝承カウンター１個を加える。IIIの後に、生け贄に捧げる。）\nI, II — あなたのライブラリーから伝説のクリーチャー・カード１枚を探し、公開し、あなたの手札に加える。その後、ライブラリーを切り直す。\nIII — ターン終了時まで、あなたがコントロールしているすべてのクリーチャーは二段攻撃を得る。指輪があなたを誘惑する。",
                Type = "エンチャント — 英雄譚",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618171&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "9c41afe8-96b9-4ff3-af09-e2ecb4a53c7d",
                    MultiverseId = 618171
                },
                Multiverseid = 618171
            },
            {
                Name = "Guerra da Última Aliança",
                Text = "(Conforme esta Saga entra e após sua etapa de compra, adicione um marcador de conhecimento. Sacrifique-a após III.)\nI, II — Procure em seu grimório um card de criatura lendário, revele-o, coloque-o em sua mão e depois embaralhe.\nIII — As criaturas que você controla ganham golpe duplo até o final do turno. O Anel tenta você.",
                Type = "Encantamento — Saga",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618432&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "3a4b4cc3-64c4-4904-9349-eaf5662744b0",
                    MultiverseId = 618432
                },
                Multiverseid = 618432
            },
            {
                Name = "最后联盟之战",
                Text = "（于此传纪进战场时及于你抓牌步骤后，加一个学问指示物。到III后牺牲之。）\nI, II — 从你牌库中搜寻一张传奇生物牌，展示该牌，将它置于你手上，然后洗牌。\nIII — 由你操控的生物获得连击异能直到回合结束。魔戒引诱你。",
                Type = "结界 ～传纪",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618693&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "c1be64b3-263f-42f4-9c11-6e5514800747",
                    MultiverseId = 618693
                },
                Multiverseid = 618693
            }
        },
        Printings = {
            "LTR",
            "PLTR"
        },
        OriginalText = "(As this Saga enters and after your draw step, add a lore counter. Sacrifice after III.)\nI, II — Search your library for a legendary creature card, reveal it, put it into your hand, then shuffle.\nIII — Creatures you control gain double strike until end of turn. The Ring tempts you.",
        OriginalType = "Enchantment — Saga",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Penny",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "7e33f35e-c2fe-5fcd-a0e3-42a811e79479"
    }
