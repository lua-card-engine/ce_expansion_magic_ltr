local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_grond_the_gatebreaker"
CARD.Description = "ce_expansion_magic_ltr_grond_the_gatebreaker_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/grond_the_gatebreaker"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON_ROUNDED
CARD.Attributes = {
        ManaCost = "{3}{B}",
        Cmc = 4,
        Colors = {
            "B"
        },
        ColorIdentity = {
            "B"
        },
        Type = "Legendary Artifact — Vehicle",
        Supertypes = {
            "Legendary"
        },
        Types = {
            "Artifact"
        },
        Subtypes = {
            "Vehicle"
        },
        Rarity = "Uncommon",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "Trample\nAs long as it's your turn and you control an Army, Grond, the Gatebreaker is an artifact creature.\nCrew 3 (Tap any number of creatures you control with total power 3 or more: This Vehicle becomes an artifact creature until end of turn.)",
        Artist = "Ramazan Kazaliev",
        Number = "89",
        Power = "5",
        Toughness = "5",
        Layout = "normal",
        Multiverseid = "616919",
        Variations = {
            "2192c411-430f-5bcf-a451-9a68c83ee325"
        },
        ForeignNames = {
            {
                Name = "Grond, mächtige Ramme",
                Text = "Verursacht Trampelschaden\nSolange es dein Zug ist und du eine Armee kontrollierst, ist Grond, mächtige Ramme, eine Artefaktkreatur.\nBemannen 3 (Tappe eine beliebige Anzahl an Kreaturen, die du kontrollierst und die zusammen Stärke 3 oder mehr haben: Dieses Fahrzeug wird bis zum Ende des Zuges zu einer Artefaktkreatur.)",
                Type = "Legendäres Artefakt — Fahrzeug",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617180&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "67a86f1f-66f4-463b-b6d4-d3ee6ad16b81",
                    MultiverseId = 617180
                },
                Multiverseid = 617180
            },
            {
                Name = "Grond, el Rompepuertas",
                Text = "Arrolla.\nMientras sea tu turno y controles un Ejército, Grond, el Rompepuertas es una criatura artefacto.\nTripular 3. (Girar cualquier cantidad de criaturas que controlas con una fuerza total de 3 o más: Este Vehículo se convierte en una criatura artefacto hasta el final del turno.)",
                Type = "Artefacto legendario — Vehículo",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617441&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "2bdcec12-d23b-47a9-9bc8-8ed12a8650bf",
                    MultiverseId = 617441
                },
                Multiverseid = 617441
            },
            {
                Name = "Broyeur, le briseur de porte",
                Text = "Piétinement\nTant que c'est votre tour et que vous contrôlez une armée, Broyeur, le briseur de porte est une créature-artefact.\nPilotage 3 (Engagez n'importe quel nombre de créatures que vous contrôlez dont la force totale est supérieure ou égale à 3 : Ce véhicule devient une créature-artefact jusqu'à la fin du tour.)",
                Type = "Artefact légendaire : véhicule",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617702&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "ebbc5274-4286-4ff0-a736-61fed8923219",
                    MultiverseId = 617702
                },
                Multiverseid = 617702
            },
            {
                Name = "Grond, lo Sfondacancelli",
                Text = "Travolgere\nFintanto che è il tuo turno e controlli un Esercito, Grond, lo Sfondacancelli è una creatura artefatto.\nManovrare 3 (TAPpa un qualsiasi numero di creature che controlli con forza totale pari o superiore a 3: Questo Veicolo diventa una creatura artefatto fino alla fine del turno.)",
                Type = "Artefatto Leggendario — Veicolo",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617963&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "9b213f1c-fe47-44e2-aabf-387dc1dc6931",
                    MultiverseId = 617963
                },
                Multiverseid = 617963
            },
            {
                Name = "城門破り、グロンド",
                Text = "トランプル\nあなたのターンであり、あなたが軍団をコントロールしているかぎり、城門破り、グロンドはアーティファクト・クリーチャーである。\n搭乗３（あなたがコントロールしている望む数のクリーチャーを、パワーの合計が３以上になるように選んでタップする：ターン終了時まで、この機体はアーティファクト・クリーチャーになる。）",
                Type = "伝説のアーティファクト — 機体",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618224&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "532111f9-db82-4711-9f3a-5ac7b5d176a9",
                    MultiverseId = 618224
                },
                Multiverseid = 618224
            },
            {
                Name = "Grond, o Quebra-portões",
                Text = "Atropelar\nEnquanto for o seu turno e você controlar um Exército, Grond, o Quebra-portões, será uma criatura artefato.\nTripular 3 (Vire qualquer número de criaturas que você controla com total de poder igual ou superior a 3: Este Veículo se torna uma criatura artefato até o final do turno.)",
                Type = "Artefato Lendário — Veículo",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618485&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "27de1c57-9cb7-4ee0-a92e-9e80cc6559ee",
                    MultiverseId = 618485
                },
                Multiverseid = 618485
            },
            {
                Name = "破门械格龙得",
                Text = "践踏\n只要是在你的回合中且你操控军队，破门械格龙得便是神器生物。\n搭载3（横置任意数量由你操控且力量总和等于或大于3的生物：此载具成为神器生物直到回合结束。）",
                Type = "传奇神器 ～载具",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618746&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "a8e62809-bc77-4187-8d5a-95f2010fba54",
                    MultiverseId = 618746
                },
                Multiverseid = 618746
            }
        },
        Printings = {
            "LTR"
        },
        OriginalText = "Trample\nAs long as it's your turn and you control an Army, Grond, the Gatebreaker is an artifact creature.\nCrew 3 (Tap any number of creatures you control with total power 3 or more: This Vehicle becomes an artifact creature until end of turn.)",
        OriginalType = "Legendary Artifact — Vehicle",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "a7f879f2-a554-5e1c-a907-18122e8135e5"
    }
