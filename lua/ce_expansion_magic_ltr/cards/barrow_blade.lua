local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_barrow_blade"
CARD.Description = "ce_expansion_magic_ltr_barrow_blade_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/barrow_blade"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON
CARD.Attributes = {
        ManaCost = "{1}",
        Cmc = 1,
        Type = "Artifact — Equipment",
        Types = {
            "Artifact"
        },
        Subtypes = {
            "Equipment"
        },
        Rarity = "Uncommon",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "Equipped creature gets +1/+1.\nWhenever equipped creature blocks or becomes blocked by a creature, that creature loses all abilities until end of turn.\nEquip {1} ({1}: Attach to target creature you control. Equip only as a sorcery.)",
        Artist = "Alexander Mokhov",
        Number = "237",
        Layout = "normal",
        Multiverseid = "617067",
        Variations = {
            "79f5544a-80f2-5d0a-b07a-4770348741f7"
        },
        ForeignNames = {
            {
                Name = "Schwert von den Hügelgräberhöhen",
                Text = "Die ausgerüstete Kreatur erhält +1/+1.\nImmer wenn die ausgerüstete Kreatur blockt oder von einer Kreatur geblockt wird, verliert jene Kreatur alle Fähigkeiten bis zum Ende des Zuges.\nAusrüsten {1} ({1}: Lege diese Karte an eine Kreatur deiner Wahl an, die du kontrollierst. Spiele Ausrüsten wie eine Hexerei.)",
                Type = "Artefakt — Ausrüstung",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617328&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "aea3359c-82f9-41a7-a724-ebabdf341569",
                    MultiverseId = 617328
                },
                Multiverseid = 617328
            },
            {
                Name = "Espada tumularia",
                Text = "La criatura equipada obtiene +1/+1.\nSiempre que la criatura equipada bloquee o sea bloqueada por una criatura, esa criatura pierde todas las habilidades hasta el final del turno.\nEquipar {1}. ({1}: Anexa este Equipo a la criatura objetivo que controlas. Activa la habilidad de equipar como un conjuro.)",
                Type = "Artefacto — Equipo",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617589&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "9dd24882-747e-49ca-8d47-5d9aa3edb742",
                    MultiverseId = 617589
                },
                Multiverseid = 617589
            },
            {
                Name = "Épée des Galgals",
                Text = "La créature équipée gagne +1/+1.\nÀ chaque fois que la créature équipée bloque ou devient bloquée par une créature, cette créature perd toutes ses capacités jusqu'à la fin du tour.\nÉquipement {1} ({1} : Attachez à une créature ciblée que vous contrôlez. N'attachez l'équipement que lorsque vous pourriez lancer un rituel.)",
                Type = "Artefact : équipement",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617850&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "9c1263c7-3018-47cd-aa76-7930801b76db",
                    MultiverseId = 617850
                },
                Multiverseid = 617850
            },
            {
                Name = "Lama dei Tumuli",
                Text = "La creatura equipaggiata prende +1/+1.\nOgniqualvolta la creatura equipaggiata blocca o viene bloccata da una creatura, quella creatura perde tutte le abilità fino alla fine del turno.\nEquipaggiare {1} ({1}: Assegna a una creatura bersaglio che controlli. Equipaggia solo come una stregoneria.)",
                Type = "Artefatto — Equipaggiamento",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618111&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "c56c5092-bd5f-4119-92f3-09674dafa8bf",
                    MultiverseId = 618111
                },
                Multiverseid = 618111
            },
            {
                Name = "塚山出土の太刀",
                Text = "装備しているクリーチャーは＋１/＋１の修整を受ける。\n装備しているクリーチャーがクリーチャー１体をブロックするかそれにブロックされた状態になるたび、ターン終了時まで、そのクリーチャーはすべての能力を失う。\n装備{1}（{1}：あなたがコントロールしているクリーチャー１体を対象とする。これをそれにつける。装備はソーサリーとしてのみ行う。）",
                Type = "アーティファクト — 装備品",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618372&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "60568d8b-c292-416d-9fd2-272f8f12d88c",
                    MultiverseId = 618372
                },
                Multiverseid = 618372
            },
            {
                Name = "Lâmina dos Túmulos",
                Text = "A criatura equipada recebe +1/+1.\nToda vez que a criatura equipada bloqueia ou é bloqueada por uma criatura, aquela criatura perde todas as habilidades até o final do turno.\nEquipar {1} ({1}: Anexe este equipamento à criatura alvo que você controla. Equipe somente como um feitiço.)",
                Type = "Artefato — Equipamento",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618633&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "5cb6bd2f-c74b-4817-9523-339601065b8c",
                    MultiverseId = 618633
                },
                Multiverseid = 618633
            },
            {
                Name = "古冢宝剑",
                Text = "佩带此武具的生物得+1/+1。\n每当佩带此武具的生物阻挡生物或被生物阻挡时，后者失去所有异能直到回合结束。\n佩带{1}（{1}：贴附在目标由你操控的生物上。只能于法术时机佩带。）",
                Type = "神器 ～武具",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618894&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "e32781a1-fc6a-44c7-ad89-efbdb68d82df",
                    MultiverseId = 618894
                },
                Multiverseid = 618894
            }
        },
        Printings = {
            "LTR"
        },
        OriginalText = "Equipped creature gets +1/+1.\nWhenever equipped creature blocks or becomes blocked by a creature, that creature loses all abilities until end of turn.\nEquip {1} ({1}: Attach to target creature you control. Equip only as a sorcery.)",
        OriginalType = "Artifact — Equipment",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "f955c7de-4395-582c-b8a3-76d963b19f83"
    }
