local CARD = CARD
CARD.Name = "ce_expansion_magic_ltr_eagles_of_the_north"
CARD.Description = "ce_expansion_magic_ltr_eagles_of_the_north_description"
CARD.Texture = "card_engine/expansions/ce_expansion_magic_ltr/eagles_of_the_north"
CARD.RearTexture = "card_engine/expansions/ce_expansion_magic_ltr/back"
CARD.CardSize = CardEngine.DEFAULT_CARD_MODELS.COMMON
CARD.Attributes = {
        ManaCost = "{5}{W}",
        Cmc = 6,
        Colors = {
            "W"
        },
        ColorIdentity = {
            "W"
        },
        Type = "Creature — Bird Soldier",
        Types = {
            "Creature"
        },
        Subtypes = {
            "Bird",
            "Soldier"
        },
        Rarity = "Common",
        Set = "LTR",
        SetName = "The Lord of the Rings: Tales of Middle-earth",
        Text = "Flying\nWhen Eagles of the North enters the battlefield, creatures you control get +1/+0 and gain first strike until end of turn.\nPlainscycling {1} ({1}, Discard this card: Search your library for a Plains card, reveal it, put it into your hand, then shuffle.)",
        Artist = "Axel Sauerwald",
        Number = "7",
        Power = "3",
        Toughness = "3",
        Layout = "normal",
        Multiverseid = "616837",
        Variations = {
            "c778f565-ef52-5425-baea-0a4fd1ac388d"
        },
        Rulings = {
            {
                Date = "2023-06-16",
                Text = "Eagles of the North's triggered ability affects only creatures you control at the time it resolves. Any creatures that come under your control later in the turn won't be affected."
            },
            {
                Date = "2023-06-16",
                Text = "Typecycling is a form of cycling. Any ability that triggers on a card being cycled also triggers on a card being typecycled. Any ability that stops a cycling ability from being activated also stops a typecycling ability from being activated."
            },
            {
                Date = "2023-06-16",
                Text = "Unlike the normal cycling ability, typecycling doesn't allow you to draw a card. Rather, it lets you search your library for a card with the type or types indicated by the ability name. For example, a card with basic landcycling lets you search for a basic land card, and a card with Wizardcycling lets you search for a Wizard card."
            }
        },
        ForeignNames = {
            {
                Name = "Adler des Nordens",
                Text = "Fliegend\nWenn die Adler des Nordens ins Spiel kommen, erhalten Kreaturen, die du kontrollierst, +1/+0 und Erstschlag bis zum Ende des Zuges.\nEbenenumwandlung {1} ({1}, wirf diese Karte ab: Durchsuche deine Bibliothek nach einer Ebene-Karte, zeige sie offen vor, nimm sie auf deine Hand und mische danach.)",
                Type = "Kreatur — Vogel, Soldat",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617098&type=card",
                Language = "German",
                Identifiers = {
                    ScryfallId = "10f70d10-5b37-4322-8097-dfc892148736",
                    MultiverseId = 617098
                },
                Multiverseid = 617098
            },
            {
                Name = "Águilas del norte",
                Text = "Vuela.\nCuando las Águilas del norte entren al campo de batalla, las criaturas que controlas obtienen +1/+0 y ganan la habilidad de dañar primero hasta el final del turno.\nCiclo de llanura {1}. ({1}, descartar esta carta: Busca en tu biblioteca una carta de Llanura, muéstrala, ponla en tu mano y luego baraja.)",
                Type = "Criatura — Soldado ave",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617359&type=card",
                Language = "Spanish",
                Identifiers = {
                    ScryfallId = "a325c895-d360-411a-b65f-075cc2c06b98",
                    MultiverseId = 617359
                },
                Multiverseid = 617359
            },
            {
                Name = "Aigles du nord",
                Text = "Vol\nQuand les Aigles du nord arrivent sur le champ de bataille, les créatures que vous contrôlez gagnent +1/+0 et acquièrent l'initiative jusqu'à la fin du tour.\nRecyclage de plaine {1} ({1}, défaussez-vous de cette carte : Cherchez dans votre bibliothèque une carte de plaine, révélez-la, mettez-la dans votre main, puis mélangez.)",
                Type = "Créature : oiseau et soldat",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617620&type=card",
                Language = "French",
                Identifiers = {
                    ScryfallId = "d92745c6-5e5c-43d2-aaeb-4f56cde940ad",
                    MultiverseId = 617620
                },
                Multiverseid = 617620
            },
            {
                Name = "Aquile del Nord",
                Text = "Volare\nQuando le Aquile del Nord entrano nel campo di battaglia, le creature che controlli prendono +1/+0 e hanno attacco improvviso fino alla fine del turno.\nCiclopianura {1} ({1}, Scarta questa carta: Passa in rassegna il tuo grimorio per una carta Pianura, rivelala e aggiungila alla tua mano, poi rimescola.)",
                Type = "Creatura — Soldato Uccello",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=617881&type=card",
                Language = "Italian",
                Identifiers = {
                    ScryfallId = "e482dd6e-228f-4312-9b41-c5c1870f66f6",
                    MultiverseId = 617881
                },
                Multiverseid = 617881
            },
            {
                Name = "北方の大鷲",
                Text = "飛行\n北方の大鷲が戦場に出たとき、ターン終了時まで、あなたがコントロールしているすべてのクリーチャーは＋１/＋０の修整を受け先制攻撃を得る。\n平地サイクリング {1} ({1}, このカードを捨てる：あなたのライブラリーから平地・カード１枚を探し、公開し、あなたの手札に加える。その後、ライブラリーを切り直す。）",
                Type = "クリーチャー — 鳥・兵士",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618142&type=card",
                Language = "Japanese",
                Identifiers = {
                    ScryfallId = "e5147272-72c9-46fd-a666-951bfef8c6fb",
                    MultiverseId = 618142
                },
                Multiverseid = 618142
            },
            {
                Name = "Águias do Norte",
                Text = "Voar\nQuando Águias do Norte entra no campo de batalha, as criaturas que você controla recebem +1/+0 e ganham iniciativa até o final do turno.\nReciclar Planície \\{1} (\\{1}, descarte este card: Procure em seu grimório um card de Planície, revele-o, coloque-o em sua mão e depois embaralhe.)",
                Type = "Criatura — Ave Soldado",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618403&type=card",
                Language = "Portuguese (Brazil)",
                Identifiers = {
                    ScryfallId = "119bf846-645d-4c82-bfd5-f244f3894560",
                    MultiverseId = 618403
                },
                Multiverseid = 618403
            },
            {
                Name = "北山群鹰",
                Text = "飞行\n当北山群鹰进战场时，直到回合结束，由你操控的生物得+1/+0且获得先攻异能。\n循环平原{1}（{1}，弃掉此牌：从你牌库中搜寻一张平原牌，展示该牌，将它置于你手上，然后洗牌。）",
                Type = "生物 ～鸟／士兵",
                Flavor = nil,
                ImageUrl = "http://gatherer.wizards.com/Handlers/Image.ashx?multiverseid=618664&type=card",
                Language = "Chinese Simplified",
                Identifiers = {
                    ScryfallId = "fe68e58c-178c-465f-a480-1db9528ad0ee",
                    MultiverseId = 618664
                },
                Multiverseid = 618664
            }
        },
        Printings = {
            "LTR"
        },
        OriginalText = "Flying\nWhen Eagles of the North enters the battlefield, creatures you control get +1/+0 and gain first strike until end of turn.\nPlainscycling {1} ({1}, Discard this card: Search your library for a Plains card, reveal it, put it into your hand, then shuffle.)",
        OriginalType = "Creature — Bird Soldier",
        Legalities = {
            {
                Format = "Alchemy",
                Legality = "Legal"
            },
            {
                Format = "Brawl",
                Legality = "Legal"
            },
            {
                Format = "Commander",
                Legality = "Legal"
            },
            {
                Format = "Duel",
                Legality = "Legal"
            },
            {
                Format = "Gladiator",
                Legality = "Legal"
            },
            {
                Format = "Historic",
                Legality = "Legal"
            },
            {
                Format = "Legacy",
                Legality = "Legal"
            },
            {
                Format = "Modern",
                Legality = "Legal"
            },
            {
                Format = "Oathbreaker",
                Legality = "Legal"
            },
            {
                Format = "Pauper",
                Legality = "Legal"
            },
            {
                Format = "Paupercommander",
                Legality = "Legal"
            },
            {
                Format = "Timeless",
                Legality = "Legal"
            },
            {
                Format = "Vintage",
                Legality = "Legal"
            }
        },
        Id = "704de46c-382b-5e06-8469-f5f5400fb671"
    }
